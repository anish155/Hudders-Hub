 ...
 *
 * NOTE: Replace the session check with your real admin auth guard.
 */
require_once '../../config/database.php';
header('Content-Type: application/json');

// ── Admin auth guard ──────────────────────────────────────────────────
// Replace this with your real admin session/role check.
// The HUDDER_ADMIN table links admin_id → user_id.
session_start();
$allowed = false;

// Accept if an admin session key is present
if (!empty($_SESSION['admin_id'])) {
    $allowed = true;
}

// Also allow via POST override during development (remove in production)
...
try {
    // 1. Verify trader is currently Pending
    $chk = oci_parse($conn,
        "SELECT user_id, status FROM TRADER WHERE trader_id = :tid"
    );
