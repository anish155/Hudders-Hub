<?php
/**
 * HuddersHub - Individual Shop Page (Butcher, Greengrocer, etc.)
 * Shows all products for one shop type with full pagination
 */
...
// Determine shop from filename
$shop = basename($_SERVER['PHP_SELF'], '.php');
$valid_shops = ['butcher', 'greengrocer', 'fishmonger', 'bakery', 'delicatessen'];
if (!in_array($shop, $valid_shops)) { header('Location: category.php'); exit; }
...
$shop_subs = [
    'butcher' => ['Beef', 'Chicken', 'Lamb', 'Pork', 'Sausages'],
    'greengrocer' => ['Fruits', 'Vegetables', 'Salad Items', 'Herbs', 'Organic Produce'],
    'fishmonger' => ['Fresh Fish', 'Shellfish', 'Smoked Fish', 'Prepared Fish', 'Frozen'],
    'bakery' => ['Bread', 'Rolls & Buns', 'Cakes', 'Pastries', 'Pies'],
    'delicatessen' => ['Cheeses', 'Antipasti', 'Dips & Spreads', 'Cured Meats', 'Gift Hampers'],
];
...
$binds = [':shop_type' => strtoupper($shop)];
if ($sub) { $where[] = "UPPER(pc.category_name) = :sub_cat"; $binds[':sub_cat'] = strtoupper($sub); }
if ($type) {
    if ($type === 'veg') {
        $where[] = "INSTR(UPPER(p.dietary_tags), 'VEGETARIAN') > 0";
    } elseif ($type === 'non-veg') {
        $where[] = "(p.dietary_tags IS NULL OR (INSTR(UPPER(p.dietary_tags), 'VEGETARIAN') = 0 AND INSTR(UPPER(p.dietary_tags), 'VEGAN') = 0))";
    } elseif ($type === 'vegan') {
        $where[] = "INSTR(UPPER(p.dietary_tags), 'VEGAN') > 0";
    } elseif ($type === 'gluten-free') {
        $where[] = "INSTR(UPPER(p.dietary_tags), 'GLUTEN FREE') > 0";
    }
}
...
// Count
$count_sql = "SELECT COUNT(*) AS total FROM PRODUCT p JOIN SHOP s ON p.shop_id = s.shop_id LEFT JOIN PRODUCT_CATEGORY pc ON p.category_id = pc.category_id WHERE $where_sql";
$count_stmt = oci_parse($conn, $count_sql);
foreach ($binds as $b => $v) { oci_bind_by_name($count_stmt, $b, $v); }
oci_execute($count_stmt);
$total_products = (int)(oci_fetch_assoc($count_stmt)['TOTAL'] ?? 0);
...
$sql = "SELECT * FROM (
    ...
    WHERE $where_sql
) WHERE rn BETWEEN :offset_start AND :offset_end ORDER BY rn";

$stmt = oci_parse($conn, $sql);
foreach ($binds as $b => $v) { oci_bind_by_name($stmt, $b, $v); }
$o_start = $offset + 1;
$o_end = $offset + $per_page;
oci_bind_by_name($stmt, ':offset_start', $o_start);
oci_bind_by_name($stmt, ':offset_end', $o_end);
oci_execute($stmt);
...
function buildUrl($overrides = []) {
    global $shop, $sub, $type, $sort, $inStock, $page;
    $params = ['shop' => $shop];
...
<head>
    ...
    <title><?php echo htmlspecialchars($shop_labels[$shop]); ?> | HuddersHub</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Back link */
        .back-link { display: inline-flex; align-items: center; gap: 6px; font-size: 14px; font-weight: 500; color: var(--text-medium-gray); margin-bottom: 20px; }
        .back-link:hover { color: var(--primary-green); }
        .back-link .material-icons-outlined { font-size: 18px; }

        /* Shop header */
        .shop-header { margin-bottom: 24px; }
        .shop-header h1 { font-family: "Google Sans Flex", sans-serif; font-size: 32px; font-weight: 700; color: var(--text-black); margin-bottom: 4px; }
        .shop-header p { font-size: 14px; color: var(--text-medium-gray); }
        ...
        .product-image { position: relative; height: 160px; background: #e8e8e8; overflow: hidden; display: flex; align-items: center; justify-content: center; }
        ...
        .product-name { font-size: 13px; font-weight: 600; color: var(--text-dark-gray); line-height: 1.3; min-height: 34px; }
        .product-price { font-size: 14px; font-weight: 700; color: var(--text-black); }
        .add-btn { width: 100%; padding: 8px; border: 1px solid var(--border-light); background: var(--bg-white); font-size: 12px; font-weight: 600; color: var(--text-dark-gray); cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 4px; font-family: "Plus Jakarta Sans", sans-serif; transition: var(--transition-smooth); border-radius: 2px; }
        ...
        .page-btn { display: inline-flex; align-items: center; justify-content: center; min-width: 34px; height: 34px; padding: 0 10px; border: 1px solid var(--border-light); background: var(--bg-white); font-weight: 600; font-size: 13px; color: var(--text-medium-gray); font-family: "Plus Jakarta Sans", sans-serif; transition: border-color 0.15s ease, color 0.15s ease; }
        .page-btn:hover { border-color: var(--primary-green); color: var(--primary-green); }
...
<body>
    ...
    <main class="page-content">
        <div class="page-wrap">
            <a href="category.php" class="back-link"><span class="material-icons-outlined">arrow_back</span> Back to all categories</a>
            <div class="shop-header">
                <h1><?php echo htmlspecialchars($shop_labels[$shop]); ?></h1>
                <p><?php echo $total_products; ?> products available</p>
            </div>

            <div class="sub-grid">
                <a href="<?php echo buildUrl(['sub' => '']); ?>" class="sub-box <?php echo !$sub ? 'active' : ''; ?>">All Categories</a>
                <?php foreach ($shop_subs[$shop] as $s): ?>
            ...
            <div style="margin-bottom: 24px; display: flex; gap: 10px; flex-wrap: wrap;">
                <a href="<?php echo buildUrl(['type' => '']); ?>" style="padding: 6px 14px; border-radius: 20px; font-size: 13px; font-weight: 600; border: 1px solid <?php echo !$type ? 'var(--primary-green)' : 'var(--border-light)'; ?>; background: <?php echo !$type ? 'var(--primary-green)' : 'transparent'; ?>; color: <?php echo !$type ? 'white' : 'var(--text-dark-gray)'; ?>;">All Types</a>
                <a href="<?php echo buildUrl(['type' => 'veg']); ?>" style="padding: 6px 14px; border-radius: 20px; font-size: 13px; font-weight: 600; border: 1px solid <?php echo $type === 'veg' ? 'var(--primary-green)' : 'var(--border-light)'; ?>; background: <?php echo $type === 'veg' ? 'var(--primary-green)' : 'transparent'; ?>; color: <?php echo $type === 'veg' ? 'white' : 'var(--text-dark-gray)'; ?>;">Vegetarian</a>
                <a href="<?php echo buildUrl(['type' => 'non-veg']); ?>" style="padding: 6px 14px; border-radius: 20px; font-size: 13px; font-weight: 600; border: 1px solid <?php echo $type === 'non-veg' ? 'var(--primary-green)' : 'var(--border-light)'; ?>; background: <?php echo $type === 'non-veg' ? 'var(--primary-green)' : 'transparent'; ?>; color: <?php echo $type === 'non-veg' ? 'white' : 'var(--text-dark-gray)'; ?>;">Non-Vegetarian</a>
                <a href="<?php echo buildUrl(['type' => 'vegan']); ?>" style="padding: 6px 14px; border-radius: 20px; font-size: 13px; font-weight: 600; border: 1px solid <?php echo $type === 'vegan' ? 'var(--primary-green)' : 'var(--border-light)'; ?>; background: <?php echo $type === 'vegan' ? 'var(--primary-green)' : 'transparent'; ?>; color: <?php echo $type === 'vegan' ? 'white' : 'var(--text-dark-gray)'; ?>;">Vegan</a>
                <a href="<?php echo buildUrl(['type' => 'gluten-free']); ?>" style="padding: 6px 14px; border-radius: 20px; font-size: 13px; font-weight: 600; border: 1px solid <?php echo $type === 'gluten-free' ? 'var(--primary-green)' : 'var(--border-light)'; ?>; background: <?php echo $type === 'gluten-free' ? 'var(--primary-green)' : 'transparent'; ?>; color: <?php echo $type === 'gluten-free' ? 'white' : 'var(--text-dark-gray)'; ?>;">Gluten Free</a>
            </div>

            <div class="section-toolbar">
                <div class="toolbar-left">Showing <strong><?php echo $from; ?>-<?php echo $to; ?></strong> of <strong><?php echo $total_products; ?></strong> products</div>
            ...
            <div style="text-align:center;padding:60px 20px;background:var(--bg-gray);border:1px solid var(--border-light);border-radius:8px;">
                <h3 style="font-size:18px;font-weight:700;margin-bottom:8px;">No products found</h3>
                <p style="font-size:14px;color:var(--text-medium-gray);margin-bottom:16px;">Try adjusting your filters.</p>
                <a href="<?php echo buildUrl(['sub'=>'','in_stock'=>0]); ?>" style="display:inline-block;padding:10px 22px;background:var(--primary-green);color:white;font-weight:600;font-size:14px;border-radius:4px;">Clear filters</a>
            </div>
            <?php else: ?>
            ...
            <div class="pagination">
                <a href="<?php echo buildUrl(['page'=>$page-1]); ?>" class="page-btn <?php echo $page<=1?'disabled':''; ?>">Prev</a>
                <?php
                $start = max(1, $page-2);
                $end = min($total_pages, $page+2);
                if ($start > 1): ?><a href="<?php echo buildUrl(['page'=>1]); ?>" class="page-btn">1</a><?php if ($start>2): ?><span class="page-btn disabled">...</span><?php endif; ?><?php endif; ?>
                <?php for ($i=$start;$i<=$end;$i++): ?><a href="<?php echo buildUrl(['page'=>$i]); ?>" class="page-btn <?php echo $i===$page?'active':''; ?>"><?php echo $i; ?></a><?php endfor; ?>
                <?php if ($end < $total_pages): ?><?php if ($end<$total_pages-1): ?><span class="page-btn disabled">...</span><?php endif; ?><a href="<?php echo buildUrl(['page'=>$total_pages]); ?>" class="page-btn"><?php echo $total_pages; ?></a><?php endif; ?>
                <a href="<?php echo buildUrl(['page'=>$page+1]); ?>" class="page-btn <?php echo $page>=$total_pages?'disabled':''; ?>">Next</a>
            </div>
            <?php endif; ?>
            <?php endif; ?>
        </div>
